...
import static org.zornco.pathvis.ClientRegistration.LineTypes.THICCCCC_LINES;
...
public class NodeParticle extends Particle {
    ...
    protected NodeParticle(ClientLevel world, double sourceX, double sourceY, double sourceZ, double targetX, double targetY, double targetZ, float red, float green, float blue, float scale) {
        ...
        this.scale = scale;
        this.setGravity(0f);
        this.lifetime = 10 * (int)Math.round(sourcePos.distanceTo(targetPos)) + 12;
    }
    ...
    private void renderOctahedron(PoseStack matrixStack, VertexConsumer ivertexbuilder) {
        ...
        for (int i: lineLoop1) {
            ...
            f1 /= f3;
            f2 /= f3;
            ivertexbuilder.vertex(matrixStack.last().pose(), (float)(x+octahedron[i].x * scale), (float)(y+octahedron[i].y * scale), (float)(z+octahedron[i].z * scale)).color(rCol, gCol, bCol, 1F).normal(matrixStack.last().normal(), f, f1, f2).endVertex();
        }
    }
    ...
    public void tick() {
        ...
        Vec3 subtract = targetPos.subtract(partPos);
        if (!(Minecraft.getInstance().player != null && Minecraft.getInstance().player.getMainHandItem().getItem() instanceof PathingVisualizerItem))
            this.remove();
    ...
    public static class FACTORY implements ParticleProvider<NodeParticleData> {
        ...
        @Override
        public Particle createParticle(NodeParticleData data, @Nonnull ClientLevel world, double sourceX, double sourceY, double sourceZ, double targetX, double targetY, double targetZ) {
            return new NodeParticle(world, sourceX, sourceY, sourceZ, targetX, targetY, targetZ, data.r(), data.g(), data.b(), data.s());
        }
    }
}
